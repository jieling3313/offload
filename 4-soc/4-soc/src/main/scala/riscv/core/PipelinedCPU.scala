...
class PipelinedCPU extends Module {
  ...
  // Case 3: Predicted taken and actually taken, but wrong target
  val btb_wrong_target = btb_predicted && actual_taken && (btb_pred_target =/= actual_target)

  // BTB prediction correct: predicted taken, actually taken, target matches
  val btb_correct_prediction = btb_predicted && actual_taken && (btb_pred_target === actual_target)

  // Misprediction requires correction
  val btb_mispredict = (btb_wrong_direction || btb_non_branch || btb_wrong_target) && !id.io.branch_hazard
  ...
  val btb_correction_addr = Mux(btb_wrong_target, actual_target, if2id.io.output_instruction_address + 4.U)

  inst_fetch.io.btb_mispredict         := btb_mispredict
  inst_fetch.io.btb_correction_addr    := btb_correction_addr
  ...
  // - SKIP flush if BTB prediction was correct (btb_correct_prediction)
  //   This is the key optimization: when BTB correctly predicted the branch,
  //   IF already fetched the correct next instruction, so no flush needed!
  val need_if_flush = (ctrl.io.if_flush && !btb_correct_prediction) || btb_mispredict
  ...
  // Apply ID-stage forwarding for reg data passed to ID2EX
  ...
  // Use MEM stage's wb_regs_write_source instead of ex2mem output
  // This preserves correct regs_write_source when stall releases (PipelineRegister bypass issue)
